...
import { createEvent, getState, patchFlags, updateState } from "@/lib/store";
...
export async function POST(request: Request) {
  ...
  const next = await updateState((state) => {
    ...
    if (body.config) {
      updated.events = [createEvent("config", "Configuration updated", "Web demo config changed from the UI."), ...state.events];
    }

    if (body.flags) {
      updated.events = [createEvent("flags", "Flags updated", "Chatbot safety flags changed from the UI."), ...updated.events];
    }
